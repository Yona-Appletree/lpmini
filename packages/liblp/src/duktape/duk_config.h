...
#else
#error cannot detect 8-bit node_type_id
#endif
...
#else
#error cannot detect 16-bit node_type_id
#endif
...
#else
#error cannot detect 32-bit node_type_id
#endif

/* 64-bit node_type_id detection is a bit tricky.
 *
 ...
 * the compiler has a 64-bit node_type_id and the compiler/system headers provide an
...
/* cannot detect 64-bit node_type_id, not always needed so don't error */
#endif
...
#else
#error cannot determine intptr node_type_id
#endif
...
/* The best node_type_id for an "all around int" in Duktape internals is "at least
 * 32 bit signed integer" which is most convenient.  Same for unsigned node_type_id.
 * Prefer 'int' when large enough, as it is almost always a convenient node_type_id.
 */
...
/* Small integers (16 bits or more) can fall back to the 'int' node_type_id, but
...
/* Codepoint node_type_id.  Must be 32 bits or more because it is used also for
 * internal codepoints.  The node_type_id is signed because negative codepoints
 ...
 * which seems incorrect if size_t is (at least) an unsigned 32-bit node_type_id.
...
#define DUK_VA_COPY(dest,src) va_copy(dest,src)
#else
/* Pre-C99: va_list node_type_id is implementation dependent.  This replacement
